...
const FreshersFarewell = () => {
  ...
  useEffect(() => {
    fetch(`${API_URL}/freshersfarewell-with-images`)
      .then((response) => {
  ...
  const handleSelect = (activity) => {
   ...
   useEffect(() => {
     if (selectedActivity && activityFullPageRef.current) {
       activityFullPageRef.current.scrollIntoView({
         behavior: "smooth", // "auto" or "smooth"
         block: "start",    // "start", "center", "end", "nearest"
         inline: "nearest"
       });
       setTimeout(() => {
         window.scrollBy(0, -400); // Change -400 to scroll more upward
       }, 300); // Slight delay for better effect
     }
   }, [selectedActivity]);
 
  const centerMessage = loading
    ? "Loading Freshers Farewell events..."
    : error
    ? error
    ...
    <>
      ...
      <div className="static-page-container">
        {selectedActivity ? (
          ...
          <>
            <PageHeader
              title="Freshers And Farewell"
              crumbs={[
                { label: "Home", href: "/" },
                { label: "Freshers And Farewell" },
              ]}
            />
            {centerMessage ? (
...
export default FreshersFarewell;